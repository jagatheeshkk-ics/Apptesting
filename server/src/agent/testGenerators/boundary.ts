import { DetectedField, DetectedModule, GeneratedTestCase } from "../../types.js";
...
function boundaryValuesFor(field: DetectedField): BoundaryCase[] {
  ...
  if (field.type === "number") {
    if (field.min !== undefined) {
      cases.push({ suffix: `min value (${field.min})`, value: String(field.min), expectation: "At minimum: accepted." });
      cases.push({
    ...
    if (field.max !== undefined) {
      cases.push({ suffix: `max value (${field.max})`, value: String(field.max), expectation: "At maximum: accepted." });
      cases.push({
    ...
    cases.push({ suffix: "zero", value: "0", expectation: "Zero should be handled explicitly, not misinterpreted." });
    cases.push({ suffix: "negative number", value: "-1", expectation: "Negative numbers should be validated per business rules." });
    cases.push({
  ...
  if (field.type === "email") {
    cases.push({ suffix: "malformed email", value: "not-an-email", expectation: "Should be rejected as invalid email." });
    cases.push({ suffix: "missing domain", value: "user@", expectation: "Should be rejected as invalid email." });
    cases.push({ suffix: "valid edge-case email", value: "a@b.co", expectation: "Short valid email should be accepted." });
  }

  if (field.type === "date") {
    cases.push({ suffix: "far past date", value: "1900-01-01", expectation: "Should be handled per business rules (accept or reject)." });
    cases.push({ suffix: "far future date", value: "2999-12-31", expectation: "Should be handled per business rules (accept or reject)." });
    cases.push({ suffix: "invalid date", value: "2024-02-30", expectation: "Invalid calendar date should be rejected." });
  }

  // Unicode / special characters relevant to boundary + encoding correctness
  cases.push({ suffix: "unicode characters", value: "テスト😀Ω", expectation: "Unicode input should be stored/displayed correctly, not mangled." });
  cases.push({
